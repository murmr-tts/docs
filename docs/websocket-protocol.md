...
After connecting, send a `config` message with your API key within **10 seconds**. The server authenticates the key, opens a backend connection, and responds with `config_ack` when the connection is authenticated and ready. You can start sending text as soon as you receive `config_ack`.

> **Plan Requirement:** WebSocket is available on **Realtime** and **Scale** plans only. Other plans receive close code `4002`.
...
| `voice` | `string` | No | -- | Saved voice ID (e.g., `voice_abc123`). Alternative to `voice_description`. |
...
Sent after the `config` message is fully authenticated. Confirms the connection is ready to accept text.
